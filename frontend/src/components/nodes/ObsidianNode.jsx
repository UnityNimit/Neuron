...
const ObsidianNode = ({ data, selected }) => {
  ...
  // --- Z-LEVEL LOD SYSTEM ---
  const zLevel = zoom < 0.3 ? 0 : zoom < 0.8 ? 1 : zoom < 1.5 ? 2 : 3;

  // --- ORBITAL PHYSICS SIZING ---
  // FIX: We MUST give the wrapper a physical size so React Flow doesn't hide it!
  const sizeClass = isFolder ? 'w-10 h-10' : isFile ? 'w-5 h-5' : 'w-3 h-3';
  
  let orbColor = 'bg-[#8b5cf6]'; // Default: Function (Purple)
  if (isFolder) orbColor = 'bg-[#4b5563]'; // Folder (Matte Gray)
  if (isFile) orbColor = 'bg-[#3b82f6]'; // File (Blue)

  // --- AI RISK HEATMAP OVERRIDES ---
  let riskGlow = '';
  if (data.risk === 'high') {
    orbColor = 'bg-[#ef4444]'; // Radioactive Red
    riskGlow = 'ring-4 ring-[#ef4444]/30 shadow-[0_0_30px_rgba(239,68,68,0.8)]';
  } else if (data.isImpacted) {
    orbColor = 'bg-[#fb923c]'; // Blast Radius Orange
  ...
  return (
    <div className={`relative flex items-center justify-center ${sizeClass} cursor-pointer`}>
      ...
      {/* 1. ZERO-POINT HANDLES */}
      {/* FIX: Absolute positioned to the dead-center of the wrapper so lines pierce the core! */}
      <Handle 
        type="target" 
        position={Position.Top} 
        className="opacity-0 absolute pointer-events-none border-none" 
        style={{ top: '50%', left: '50%', transform: 'translate(-50%, -50%)' }} 
      />

      {/* 2. THE PLANET (Visual Orb) */}
      <div 
      ...
      <div className="absolute top-full left-1/2 -translate-x-1/2 mt-1 flex flex-col items-center pointer-events-none z-20 overflow-visible">
        ...
        {/* Z-LEVEL 1 & 2: Names and Signatures */}
        {(zLevel === 1 || zLevel === 2) && (
        ...
        {/* Z-LEVEL 3: AI Semantic Peel (The "Code Peek") */}
        {zLevel === 3 && (
          <div className="mt-2 w-[340px] bg-[#141414] border border-[#333] rounded-md shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-100">
            ...
            {data.aiSummary && (
              <div className="bg-[#1a202c] border-b border-[#2d3748] px-3 py-2">
                <span className="text-[#93c5fd] font-sans text-[10px] leading-relaxed">
                  [AI]: {data.aiSummary}
                </span>
              </div>
            )}
            ...
            <div className="px-3 py-2 bg-[#0f0f0f]">
              <pre className="text-[#999] font-mono text-[9px] whitespace-pre-wrap leading-tight">
