...
export default function App() {
    ...
    return (
      <div className="w-screen h-screen bg-[#0f0f0f] flex flex-col items-center justify-center relative overflow-hidden">
        ...
        <div className="bg-[#141414] border border-slate-800 p-10 rounded-2xl shadow-[0_0_50px_rgba(0,0,0,0.5)] flex flex-col items-center z-10">
          <h1 className="text-white font-bold text-2xl mb-8">NEURON</h1>
          <button onClick={() => supabase.auth.signInWithOAuth({ provider: 'google' })} className="bg-white text-black px-4 py-3 rounded font-bold">Continue with Google</button>
        </div>
      </div>
    );
  }
  ...
  if (!workspace.isGraphLoaded || !isCompilerReady) {
    return <div className="w-screen h-screen bg-[#0f0f0f] flex items-center justify-center text-white">Loading Neuron Engine...</div>;
  }

  const centerSize = 1000 - (layout.sidebar ? 200 : 0) - (layout.stdin ? 200 : 0);
  ...
  return (
    <div className="w-screen h-screen bg-[#0f0f0f] flex flex-col font-sans text-slate-300 overflow-hidden">
      ...
      <div className="flex flex-row flex-grow overflow-hidden">
        ...
        <Group orientation="horizontal" className="flex-grow overflow-hidden" autoSaveId="neuron-layout-v5">
          {layout.sidebar && (
            <>
          ...
          <Panel id="main-canvas" order={2} defaultSize={centerSize} minSize={200} className="flex flex-col bg-[#0f0f0f]">
            <Group orientation="vertical" autoSaveId="neuron-vertical-v5">
              <Panel id="canvas-area" order={1} defaultSize={layout.terminal ? 75 : 100} minSize={40} className="flex flex-col relative">
              ...
              {layout.terminal && (
                <>
                  ...
                  <Panel id="terminal-area" order={2} defaultSize={200} minSize={15} maxSize={300} className="bg-[#181818] flex flex-col font-mono text-sm">
